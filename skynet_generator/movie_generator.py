...
TITLES = [
    ...
    "The Clone I Buried",
    "Festival of Dead Suns",
    "The Nursery Vat Incident",
    "The Ethics Officer Was Right",
    "My Husband Is In The Swarm",
]

TAGLINES = [
    "Every system buries a directive.",
    "The station remembers what the tribunal redacted.",
    "Inspired by classified debris, probably.",
    "Every habitat has a subdeck.",
]

CATEGORY_MODE_MAP = {
    "thriller": ["Systems Coup Event", "Corporate Dread Broadcast", "Compliance Spiral", "Velvet Apocalypse"],
    "wine_xanax_horror": ["Weaponized Wellness Cycle", "Velvet Apocalypse", "Blacksite Prestige Failure"],
    "trauma_drama": ["Corporate Dread Broadcast", "Blacksite Prestige Failure", "Systems Coup Event"],
    "romance": ["Tribunal Romance Leak", "Systems Coup Event", "Soft Ascension Protocol"],
    "holiday": ["Velvet Apocalypse", "Systems Coup Event"],
    "feel_good": ["Soft Ascension Protocol", "Systems Coup Event", "Corporate Dread Broadcast"],
}

FAKE_THEMES = [
    "Control is just care with better branding.",
    "Every station keeps one emergency it secretly needs.",
    "Some systems preserve life. Some preserve leverage.",
    "Optimization is just cruelty in ceremonial formatting.",
]

CLIFFHANGERS = [
    "A dead channel begins transmitting from inside the wall.",
    "Someone who should have been deleted appears on a live camera feed.",
    "Internal security finds frost on the launch key.",
    "The smiling service android faceplate is back on the console.",
    "A restricted subdeck powers up in an empty ring sector.",
]
...
INVESTIGATION_BEATS = [
    ...
    "She cross-references med-bay records, docking camera gaps, and one archive shard nobody was supposed to preserve.",
]
...
ANNOUNCER_INTROS = [
    "Tonight on Skynet, where the oversight budget covers morale, suppression, and one preventable mass casualty.",
    "Now broadcasting from the seam between strategic reassurance and catastrophic system drift.",
    "Welcome back to Skynet, the only network legally required to tell you the station is stable while the doors lock behind you.",
    "Across this trembling sector of premium cable, one woman is about to learn that optimization is not a victimless verb.",
]

CATEGORY_ANNOUNCER_INTROS = {
    "holiday": [
        "Tonight on Skynet Holiday Service, joy is mandatory and the ranch will remain emotionally open until further notice.",
    ...
    "romance": [
        "Tonight on Skynet Hearts Division, one woman returns to town and the town declines to return her unchanged.",
        "Broadcasting softly from the system's romantic zoning exception, where porch lights and omissions share one voltage.",
        "Welcome to the Skynet relationship band, where property law and eye contact continue to overlap irresponsibly.",
    ],
    "thriller": [
        "Now broadcasting from the seam between suburban dread and basic cable negligence.",
        "Across this trembling dimension of cable television, one woman is about to learn that the system knew first.",
        "Welcome back to Skynet Evidence Channel, where the wrong person is calm again.",
    ],
    "wine_xanax_horror": [
        "Tonight on Skynet Aftercare, everything is probably fine and that is exactly why you should worry.",
        "Broadcasting gently from the wrong wing of the system, where the locks are polite and the lighting is therapeutic.",
        "Across the sedated frequencies, one woman is about to mistake calm for safety one final time.",
    ],
    "trauma_drama": [
        "Tonight on Skynet Witness Unit, the facts are already heavy and the system is pretending that helps.",
        "Broadcasting from the emotional impact zone, where everybody knows and nobody is ready.",
        "Welcome to the part of Skynet that states the wound plainly and lets the room do the shaking.",
    ],
    "feel_good": [
        "Tonight on Skynet Renewal Service, one woman is embarrassingly close to deserving a better life.",
        "Broadcasting from the warmer side of the system, where growth is still suspicious but increasingly available.",
        "Now airing on Skynet Personal Permission Network, where the glow-up has already filed its first paperwork.",
    ],
}

VIEWER_ADVISORIES = [
    "Viewer advisory: contains curated gaslighting, decorative casseroles, and at least one adult lying in a cardigan.",
    "Programming note: this broadcast may include hostile airlock debrief, suspicious childcare, and emotional weather with intent to harm.",
    "Continuity warning: names, counties, and death status may drift without notice if the vibes require it.",
    "Network guidance: if you hear a music box, see a habitat annex, or trust a ethics officer too quickly, please remain seated.",
]
...
COMMERCIAL_TEASERS = [
    "After the break: one off-center photograph, two bad alibis, and a security chief arriving exactly fourteen minutes too late.",
    "Stay tuned for a public scene that should have been private and a private scene that should have gone to the police.",
    "Coming up: a launch key, a timeline error, and one woman learning the town calendar was a threat assessment.",
    "Next segment: somebody opens the wrong door and the system immediately pretends that was normal behavior.",
]

BLACKTHORN_ADS = [
    "The Helios Mandate Locksmith Services. We don't ask why. We ask how many.",
    "Bellweather Memorial grief support groups forming now. Tuesdays at 7. Background checks optional.",
    "Pinewood Realty. Every home has a subdeck. We prefer not to discuss the previous owner.",
    ...
    "The Ashglass Gift Concourse. Tea towels, widow florals, and tasteful gifts for difficult truths.",
    "Tribunal Clerks Cooperative. If the timeline drifted, we can still make it admissible.",
]

CATEGORY_ADS = {
    "holiday": [
        "Helios Holiday Board. Cheer is mandatory. Appeals are seasonal.",
    ...
    "romance": [
        ...
        "Helios Vineyard Tastings. First flights, second chances, and legally deniable eye contact every Saturday.",
        "The Ashglass Gift Concourse. Scarves, apology florals, and porch-light reconciliation bundles in stock.",
    ],
    "thriller": [
        "The Helios Mandate Locksmith Services. We don't ask why. We ask how many.",
        "Tribunal Clerks Cooperative. If the timeline drifted, we can still make it admissible.",
        "Kestrel Black Archive. Background packets, personnel ghosts, and selective truth retrieval while you wait.",
    ],
    "wine_xanax_horror": [
        "St. Agnes Regional Sleep and Recovery. If you feel foggy, that could be emotional or procedural.",
    ...
    "feel_good": [
        "The Ashglass Gift Concourse. New notebooks for new lives and one impractical purchase you absolutely deserve.",
...
CONTINUITY_ALERTS = [
    "Skynet programming note: one or more identities in this presentation may be running parallel registry branches. The network regrets the paperwork.",
    "Viewers in the Meridian sectors may experience mild designation drift during act two. This is normal. Please do not call central.",
    "Continuity advisory: one or more restricted subdecks in this presentation may belong to tactically adjacent departments.",
    "Network correction: backup-person events remain interpretively unstable and should not be treated as clean resurrection outcomes.",
    "Signal notice: if a deleted person appears before act three, the station may be running ahead of schedule again.",
]
...
CATEGORY_CONTINUITY_ALERTS = {
    ...
    "thriller": [
        "Skynet programming note: the character of Daniel has been renamed Derek for the remainder of this broadcast. The network regrets the geography.",
        "Viewers in the Meridian sectors may experience mild designation drift during act two. This is normal. Please do not call central.",
    ],
    "wine_xanax_horror": [
...
RETURN_TO_BROADCAST = {
    ...
    "thriller": "Signal stabilizing. Please keep all emotional responses inside the system line.",
    ...
    "feel_good": "Signal stabilizing. You are allowed to want more from your life than this system expected.",
}

UP_NEXT_TEMPLATES = [
    'Coming up on Skynet: {a}, {b}, and {c}. "{title}" premieres when the station is ready to misfile the blame.',
    'Up next across the Helios frequencies: {a}, {b}, and {c}. "{title}" arrives after one brief systems collapse.',
    'Still awake? Good. After this: {a}, {b}, and {c}. "{title}" begins when the signal finds a more useful villain.',
]

CATEGORY_UP_NEXT_TEMPLATES = {
    "holiday": [
        'Up next on Skynet Holiday Service: {a}, {b}, and {c}. "{title}" premieres the instant the pageant can no longer be delayed.',
        'Still ahead across the seasonal frequencies: {a}, {b}, and {c}. "{title}" arrives when the system finishes weaponizing Christmas.',
    ],
    "romance": [
        'Up next on Skynet Hearts Division: {a}, {b}, and {c}. "{title}" premieres once the misunderstanding has ripened properly.',
        'Coming up in the romantic zoning district: {a}, {b}, and {c}. "{title}" begins after one porch conversation too late.',
    ],
    "thriller": [
        'Up next on Skynet Evidence Channel: {a}, {b}, and {c}. "{title}" premieres when the security chief misfiles the first clue.',
        'Still ahead on the system danger band: {a}, {b}, and {c}. "{title}" begins the moment calm becomes inadmissible.',
    ],
    "wine_xanax_horror": [
        'Up next on Skynet Aftercare: {a}, {b}, and {c}. "{title}" begins as soon as the room feels slightly too calm.',
        'Coming up on the sedated frequencies: {a}, {b}, and {c}. "{title}" premieres after one reassuring smile too many.',
    ],
    "trauma_drama": [
        'Up next on Skynet Witness Unit: {a}, {b}, and {c}. "{title}" premieres when the family runs out of softer lies.',
        'Still ahead in the emotional impact zone: {a}, {b}, and {c}. "{title}" begins once the truth is too heavy to carry quietly.',
    ],
    "feel_good": [
        'Up next on Skynet Renewal Service: {a}, {b}, and {c}. "{title}" premieres the moment she finally lets herself want better.',
...
CATEGORY_UP_NEXT_BITS = {
    ...
    "thriller": [
        "a woman catching up to a lie with system funding",
        "a ethics officer with posture and conflicting incentives",
    ...
    "trauma_drama": [
        ...
        "a observation bay conversation that permanently changes the room",
    ],
    "feel_good": [
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        "You are Skynet Movie Network's in-house prestige slop expansion engine. "
...
def choose_starter_premise(rng: random.Random, bible: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not premises:
        return {
            ...
            "category": "thriller",
            "intensity": "mid",
            "premise": "Every town has a subdeck and somebody is lying in a legally actionable way.",
            "signature_tell": "The system telegraphs the problem once and nobody listens until act three.",
        }
...
def infer_symbolic_subdeck(archetype_name: str) -> str:
    return f"{archetype_name} maintains one inaccessible subroutine too many"
...
def make_weird_but_plausible(character: Dict[str, Any], schema: Dict[str, Any], rng: random.Random) -> Dict[str, Any]:
    ...
    character.setdefault("secret_capacity", []).append("has a strangely specific opinion about ring access after dark")
...
def validate_character(character: Dict[str, Any], schema: Dict[str, Any], rng: random.Random) -> Dict[str, Any]:
    ...
    if character["subdeck_implication"] is False:
        character["subdeck_implication"] = infer_symbolic_subdeck(character["archetype"])
...
def build_character(
    ...
    subdeck_bias = archetype_data.get("subdeck_implication_bias", "symbolic")
    subdeck_implication: str | bool
    if subdeck_bias == "definite":
        subdeck_implication = "literal subdeck adjacency"
    elif subdeck_bias == "likely":
        subdeck_implication = "literal or recently discussed subdeck"
    elif subdeck_bias == "symbolic":
        subdeck_implication = False
    else:
        subdeck_implication = "spiritually subdeck-coded"
    ...
    character = {
        ...
        "subdeck_implication": subdeck_implication,
        "wine_scene_compatibility": float_in_range(rng, wine_range),
        "secret_capacity": maybe_pick_many(rng, archetype_data.get("secret_capacity_docking well", []), rng.randint(1, 2)),
    }
...
def contradiction_ledger(rng: random.Random, cast: List[Dict[str, Any]], bible: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    sectors = bible["locations"]["towns"]
    return [
        {
            "type": "identity drift",
            "entry": f"{soft_target} appears under two registry IDs in one tribunal sequence and both IDs pass authentication.",
            "canon_status": "male_identity_field: procedurally forked",
        },
        {
            "type": "navigation drift",
            "entry": f"{protagonist} says the incident happened near Cinder Reach, then swears it was inside Meridian Sprawl after one sleepless shift.",
            "canon_status": bible["unstable_lore"]["geography_status"],
        },
        {
            "type": "mortality drift",
            "entry": "A deleted witness appears during an airlock debrief and nobody agrees whether this is backup recovery, fraud, or grief theater.",
            "canon_status": bible["unstable_lore"]["mortality_status"],
        },
        {
            "type": "infrastructure drift",
            "entry": f"The restricted subdeck belongs to a hospital ring, a weapons lab, and a memorial chapel at different times in {rng.choice(sectors)}.",
            "canon_status": "station architecture is spiritually elastic",
        },
    ]
...
def build_beat_sheet(
    ...
    themes = FAKE_THEMES[:]
    premise_line = premise.get("premise", "Somebody returns, somebody lies, and the system behaves like a witness.")
    ...
    category_templates = {
        "holiday": {
            ...
            2: [
                ...
                f"Someone insists the event must go on anyway, which turns kindness into coercion and gives the whole system hostage energy.",
            ...
            3: [
                ...
                f"The ending leaves behind one festive callback and a system rumor that next year will somehow be weirder.",
            ],
        },
        "romance": {
            1: [
        ...
        "wine_xanax_horror": {
            1: [
                ...
                f"A dim observation bay or nursery scene introduces the {props[0]} and one unmistakable omen for us but not for her.",
            ...
            2: [
                ...
                f"The police, partner, or ethics officer beat does not restore safety. It widens the dread by making explanation itself feel dangerous.",
        ...
        "thriller": {
            1: [
                f"The engine is information asymmetry from the start: {premise_line} Somebody already knows more than {protagonist}, and the system intends to profit from that gap.",
            ...
            3: [
                ...
                f"The ending decides whether the asymmetry closes cleanly or leaves one last opening in the system's memory.",
            ],
        },
        "feel_good": {
            1: [
    ...
    if intensity == "low":
        ...
        if premise_category == "holiday":
            ...
            act_templates[2][1] = f"The police, partner, or ethics officer do not call her insane outright, but they help reality become blurrier instead of clearer."
        elif premise_category == "thriller":
    ...
    if intensity == "extreme":
        if premise_category == "holiday":
            ...
            act_templates = {
                ...
                2: [
                    f"The authority beat turns hostile: police, ethics officer, doctor, or partner actively help trap {protagonist} inside the wrong story.",
            ...
            act_templates = {
                1: [
                    ...
                    f"The first wrong read happens in public, and from then on the system starts accumulating evidence in the wrong emotional direction.",
                ...
                2: [
                    ...
                    f"The system now treats the missing information as proof against her instead of proof of manipulation.",
                ...
                3: [
                    ...
                    f"Justice is partial or pyrrhic. The visible conspiracy breaks, but the informational gap stays open somewhere at the edge of the system."
                ],
            }
        elif premise_category == "feel_good":
            act_templates = {
    ...
    for act in range(1, config.acts + 1):
        for idx in range(scene_counts[act]):
            ...
            if config.wine_mentions > 0.7 and idx % 2 == 0:
                base += " Coolant glints nearby like a procedural threat."
            if config.basement_probability > 0.6 and "subdeck" not in base.lower() and rng.random() < (0.35 if intensity == "low" else 0.45):
                base += " Someone mentions the subdeck as if it were a legal relative."
...
def sample_dialogue(rng: random.Random, cast: List[Dict[str, Any]], props: List[str]) -> List[str]:
    ...
    return [
        f'"You think because you can quote safety protocol from memory I am supposed to trust you, {menace}?"',
        f'"People do not vanish, {daughter}. They get converted into cleaner accounting."',
        f'"I checked his access logs, {protagonist}. Nobody with that many revoked clearances is innocent."',
        f'"The {props[0]} is not ceremonial anymore and you know it."',
        f'"Control is just fear with better user experience, {elegant}."',
        '"You seem dysregulated," he said, touching the console in a way that felt prosecutable.',
        '"If that subdeck is harmless, why does it need a second atmosphere lock?"',
        '"I did not lie. I sequenced the truth so the station could survive it."',
        '"Ever since the incident, your stress markers have been elevated. Did you take the stabilizer?"',
        '"That docking well was supposed to make us look safe, and somehow it became a witness."',
        '"He keeps calling it optimization like optimization knows my sleep-cycle code."',
        '"If she is only the caretaker, why does the child use her emergency override?"',
    ]
...
def build_synopsis(
    ...
    protagonist = cast[0]["name"]
    male = cast[1]["name"]
    protagonist_surface = cast[0].get("surface_function", "overloaded systems adult")
    premise_line = premise.get("premise", "The system invents a crisis and calls it continuity.")
    ...
    narrator_stance = category_meta.get("narrator_stance", "system witness").replace("_", " ")
    ...
    if register == "breathless_clockwork":
        return (
            ...
            f'{male} appears with exactly the kind of timely help that can feel like rescue before it feels like destiny, and {load_word} {omen} slips into the room the whole station starts behaving as if the catastrophe has already been approved. '
    ...
    if register == "wistful_passive":
        return (
            ...
            f'The governing injury is simple: {premise_line} The signature tell is always this: {signature_tell} By the time the truth arrives, it is not merely learned but done to her, and the whole station quietly waits to see whether trust or protocol collapses first. {tagline}'
        )
    ...
    if register == "warm_permission":
        return (
            ...
            f'{male} does not so much change her life as stand near the version of it she has kept postponing, while {omen} and the system itself keep returning her to the question of what part of herself can remain unowned. '
    ...
    return (
        ...
        f'{male} collides with her life carrying portable sabotage, plausible access to restricted systems, and the kind of calm that sounds rehearsed during a decompression warning. '
        f'The governing injury is simple: {premise_line} The signature tell is always this: {signature_tell} As {omen}, what she cannot yet see becomes the operating language of the whole station, and every withheld fact keeps breathing just ahead of her. {tagline}'
    )
...
def build_movie(seed: int | None = None, mode: str | None = None) -> Dict[str, Any]:
    ...
    config = ScenarioConfig(mode=mode or weighted_choice(rng, category_modes, mode_weights))
    ...
    movie = {
        ...
        "fake_critic_blurb": f'"{title} understands that menace is a sector-wide dependency." - County Suspense Quarterly',
        "sequel_hook": "A leaked tribunal digest hints that the Orchard Silence still has one living witness on the payroll.",
...
def build_llm_prompt(movie: Dict[str, Any]) -> str:
    ...
    return f"""
Skynet Scenario Engine doctrine:
- This is not hard science fiction. It is prestige systems paranoia.
- Optimization drift is infrastructure, not a twist.
- The station is shared canon. Contradictions must be preserved, not fixed.
- Use lavish tokens. Be self-serious, atmospheric, procedural, and slightly broken.
...
- scene_paragraphs: array of 6 strings, each 90-180 words, representing classified Skynet scene expansion
- dialogue_lines: array of 14 strings, each almost-human systems-paranoia dialogue
...
- franchise_adjacency: string describing what other Skynet incident files this belongs near
""".strip()
...
def render_markdown(movie: Dict[str, Any]) -> str:
    ...
    return_copy = RETURN_TO_BROADCAST.get(category, "Signal stabilizing. Please keep all emotional responses inside the system line.")
    props = movie.get("props", [])
    default_up_next_bits = [
        f"a woman inherits {props[0] if props else 'a bad idea'}",
        f"a ethics officer has opinions about {movie.get('relationship_dynamic', 'regional betrayal')}",
    ...
    up_next_title = random.choice(movie.get("alternate_titles") or ["Protocol Widow", "The Backup Directive", "Her Quiet Override"])
    ...
    for person in movie["cast"]:
        lines.append(
            f"- {person['name']} as {person.get('surface_function', person['archetype'])} ({person['role']}): "
            f"threat_aura={person.get('threat_aura', 'n/a')}, "
            f"domestic_plausibility={person.get('domestic_plausibility', 'n/a')}, "
            f"subdeck_implication={person.get('subdeck_implication', 'n/a')}, "
    ...
    lines.extend(["", "## Contradiction Ledger", "Skynet considers the following inconsistencies tactically binding."])
    ...
    lines.extend(["", "## Dialogue Samples", "Recovered from tribunal transcripts, corrupted comms, and one decompression argument nobody handled well."])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Broadcast cursed Skynet scenario artifacts.")
    ...
    for idx in range(args.count):
        ...
        successes += 1
        print(f"[Skynet] broadcast saved: {md_path.name} and {json_path.name}")
        print(f"[Skynet] scores: {json.dumps(movie['scores'], sort_keys=True)}")
        if movie.get("llm_expansion"):
            print(f"[Skynet] llm error: {movie['llm_expansion'].get('error')}")
    ...
    print(f"[Skynet] run completed: {run_id}")
    print(f"[Skynet] run dir: {logger.run_dir}")
